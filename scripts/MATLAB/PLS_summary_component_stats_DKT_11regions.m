function PLS_summary_component_stats(GENEdata_root, MRIdata_root,...
    ncomp, nperm, perm_type, working_dir, coords)
...
MRIdata=MRIdata([30,25,29,7,22,24,10,20,28,19,16])';
...
%make empty matrix for recalculated component weights values
permweights = zeros(119,nperm);
...
if strcmpi(perm_type, 'spatial')
    ...
    for j=1:nperm
        ...
        [~,~,XSp,~,~,PCTVARr] = plsregress(X,Y(Yp(:,j)),ncomp);
        % get and store variance the components explain in Y
        temp=100*PCTVARr(2,:);
        VarCr(j,:)=temp;
        %get and store weights of components
        %weighttemp=XSp;
        %permweights(:,j)=weighttemp;
        % report progress
        if mod(j,100) == 0
            disp(num2str(j))
        end
...
elseif strcmpi(perm_type,'random')
    ...
    for j=1:nperm
        ...
        [~,~,XSp,~,~,PCTVARr]=plsregress(X,Yp,ncomp);
        % get and store variance the components explain in Y
        temp=100*PCTVARr(2,:);
        VarCr(j,:)=temp;
        %get and store weights of components
...
save('listvariance.mat','VarCr');
save('repeatedweights.mat','permweights');
...
% open a file 
fid1 = fopen(fullfile(working_dir,[output_root '_11regions.csv']),'w');
